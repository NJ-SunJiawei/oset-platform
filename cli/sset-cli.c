...
static void *msg_thread_run(esl_thread_t *me, void *obj)
{
	...
	while(thread_running && handle->connected) {
		...
		if (warn_stop) {
			if (aok) {
				clear_line();
				output_printf("Type /exit or /quit or /bye to exit.\n\n");
				redisplay();
			}
			warn_stop = 0;
		}
	...
	if ((*cmd == '/' && cmd++) || !strncasecmp(cmd, "...", 3)) {
		...
		if (!strcasecmp(cmd, "exit") ||
			...
			if (tmp_debug > -1 && tmp_debug < 8) {
				...
				output_printf("sset_cli debug level set to %d\n", tmp_debug);
			} else {
				output_printf("sset_cli debug level must be 0 - 7\n");
			}
		} else {
...
static void set_fn_keys(cli_profile_t *profile)
{
	...
	profile->console_fnkeys[2] = "show modules";
	profile->console_fnkeys[3] = "";
	profile->console_fnkeys[4] = "";
	profile->console_fnkeys[5] = "reloadxml";
	profile->console_fnkeys[6] = "/log console";
	profile->console_fnkeys[7] = "/log debug";
	profile->console_fnkeys[8] = "";
...
int main(int argc, char *argv[])
{
...
#endif
#ifndef WIN32
	char hfile[512] = "/usr/local/install/sset/cli/sset_cli_history";
	char cfile[512] = "/usr/local/install/sset/cli/sset_cli.conf";
	char dft_cfile[512] = "/usr/local/install/sset/cli/sset_cli.conf";
#else
	char hfile[512] = "sset_cli_history";
	char cfile[512] = "sset_cli.conf";
	char dft_cfile[512] = "sset_cli.conf";
#endif
	...
	if (home) {
		snprintf(hfile, sizeof(hfile), "%s/.sset_cli_history", home);
		snprintf(cfile, sizeof(cfile), "%s/.sset_cli_conf", home);
	}
	signal(SIGINT, handle_SIGINT);
#ifdef SIGTSTP
	signal(SIGTSTP, handle_SIGINT);
 ...
 connect:
	...
	print_banner(stdout, is_color);
	esl_log(ESL_LOG_INFO, "SSET CLI Ready.\nenter /help for a list of commands.\n");
	output_printf("%s\n", handle.last_sr_reply);
	while (running > 0) {
		int r;
