                                ...
                                uint8_t*                e_bits)
{
  ...
  for (uint32_t r = 0; r < cfg.C; r++) {
    ...
    if (data != NULL) {
      ...
      // Attach code block CRC if required如果需要，附加代码块CRC
      if (cfg.L_cb) {
                         ...
                         srsran_sch_tb_res_nr_t* res)
{
  ...
  for (uint32_t r = 0; r < cfg.C; r++) {
    ...
    // Skip CB if mask indicates no transmission of the CB如果掩码指示CB没有传输，则跳过CB
    if (!cfg.mask[r]) {
      if (decoded) {
        cb_ok++;
      }
    ...
    // Select rate matching output sequence number of bits选择速率匹配输出序列位数
    uint32_t E = sch_nr_get_E(&cfg, j);
    j++;

    // Skip CB if it has a matched CRC如果CB具有匹配的CRC，则跳过CB
    if (decoded) {
    ...
    // Pack and count CRC OK only if CRC is match仅当CRC匹配时，打包和计数CRC正常
  ...
  // Set average number of iterations设置平均迭代次数
  if (cfg.C > 0) {
    res->avg_iter = (float)nof_iter_sum / (float)cfg.C;
  } else {
    res->avg_iter = NAN;
  ...
  // Not all CB are decoded, skip TB union and CRC check并非所有CB都被解码，跳过TB联合和CRC检查
  if (cb_ok != cfg.C) {
    return SRSRAN_SUCCESS;
  }
  ...
  for (uint32_t r = 0; r < cfg.C; r++) {
    ...
    // Subtract TB CRC from the last code block从最后一个代码块减去TB CRC
    if (r == cfg.C - 1) {
      cb_len -= cfg.L_tb;
    }
    ...
    // Compute TB CRC for last block计算最后一个块的TB CRC
  ...
  // Calculate TB CRC from packed data根据打包数据计算TB CRC
  if (cfg.C == 1) {
