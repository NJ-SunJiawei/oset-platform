...
#define OSET_LOG2_DOMAIN   "app-gnb-librrcMsg"
...
int oset_rrc_decode(const asn_TYPE_descriptor_t *td, _Out_ void *message, _In_ oset_pkbuf_t *pkbuf)
{
    ...
    rv = oset_asn_uper_decode(td, message, sizeof(*message), pkbuf);
    if (rv != OSET_OK) {
...
int oset_rrc_decode2(const asn_TYPE_descriptor_t *td, _Out_ void *message, _In_ void *data, _In_ size_t len)
{
    ...
    rv = oset_asn_uper_decode2(td, message, sizeof(*message), data, len);
    if (rv != OSET_OK) {
        oset_warn("Failed to decode NGAP-PDU");
        return rv;
    }
