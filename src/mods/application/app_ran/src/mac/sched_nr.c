...
{
	oset_assert(3 == argc);
	sched_nr *scheluder = (sched_nr *)argv[0];
	rar_info_t *rar_info = (rar_info_t *)argv[1];
	sched_nr_ue *u = (sched_nr_ue *)argv[2];
	...
	if (sched_ue_nr_find_by_rnti(rnti)) {
		...
		cc_worker_dl_rach_info(&scheluder->cc_workers[cc], rar_info);
	} else {
...
int sched_nr_dl_rach_info(sched_nr *scheluder, rar_info_t *rar_info)
{
	...
	dl_rach_info.argv[0] = scheluder;
	dl_rach_info.argv[0] = rar_info;//need free
