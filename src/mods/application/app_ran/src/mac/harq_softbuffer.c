...
void harq_softbuffer_pool_init(harq_softbuffer_pool *harq_pool, uint32_t nof_prb, uint32_t batch_size, uint32_t init_size)
{
	...
	harq_pool->tx_pool[idx] = oset_malloc(sizeof(*harq_pool->tx_pool[idx]) * init_size);
	...
	harq_pool->rx_pool[idx] = oset_malloc(sizeof(*harq_pool->rx_pool[idx]) * init_size);
	for(i = 0, i < init_size; ++i){
...
tx_harq_softbuffer *harq_softbuffer_pool_get_tx(harq_softbuffer_pool *harq_pool, uint32_t nof_prb)
{
	...
	tx_buffer = &harq_pool->tx_pool[idx][oset_pool_index(&harq_pool->tx_id_pool[idx], buffer_id)];
...
rx_harq_softbuffer *harq_softbuffer_pool_get_rx(harq_softbuffer_pool *harq_pool, uint32_t nof_prb)
{
	...
	rx_buffer = &harq_pool->rx_pool[idx][oset_pool_index(&harq_pool->rx_id_pool[idx], buffer_id)];
