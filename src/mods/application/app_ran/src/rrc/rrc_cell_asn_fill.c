...
int fill_init_dl_bwp_from_enb_cfg(rrc_nr_cfg_t *cfg, rrc_cell_cfg_nr_t *cell_cfg, ASN_RRC_BWP_DownlinkDedicated_t *init_dl_bwp)
{
	...
	for(int i = 0; i< 1; i++) {
		struct ctrl_res_set_s *coreset2_data = &cell_cfg->pdcch_cfg_ded.ctrl_res_set_to_add_mod_list[i];
	...
	for(int i = 0; i< 1; i++) {
		struct search_space_s *ss2_data = &cell_cfg->pdcch_cfg_ded.search_spaces_to_add_mod_list[i];
