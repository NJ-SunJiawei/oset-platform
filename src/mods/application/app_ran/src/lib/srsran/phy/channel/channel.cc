...
channel::channel(args_t& channel_args, uint32_t _nof_channels) :
{
  ...
  if (_nof_channels > SRSRAN_MAX_CHANNELS) {
    oset_error("Error creating channel object: maximum number of channels exceeded (%d > %d)",
            _nof_channels,
            SRSRAN_MAX_CHANNELS);
    return;
  }
  ...
  for (uint32_t i = 0; i < nof_channels; i++) {
    ...
    if (channel_args.fading_enable && !channel_args.fading_model && strstr(channel_args.fading_model, "none") &&
        ret == SRSRAN_SUCCESS) {
      fading[i] = (srsran_channel_fading_t*)calloc(sizeof(srsran_channel_fading_t), 1);
      ret       = srsran_channel_fading_init(fading[i], srate_max, channel_args.fading_model, 0x1234 * i);
    } else {
      fading[i] = nullptr;
    }
  ...
  if (ret != SRSRAN_SUCCESS) {
    oset_error("Error: Creating channel");
  }
}
  ...
  if (current_srate != srate) {
    for (uint32_t i = 0; i < nof_channels; i++) {
      if (fading[i]) {
        ...
        srsran_channel_fading_init(fading[i], srate, args.fading_model, 0x1234 * i);
      }
