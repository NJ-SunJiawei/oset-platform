 ...
 */
#define SRSRAN_NSLOTS_PER_FRAME_NR(NUM) (SRSRAN_NSLOTS_PER_SF_NR(NUM) * SRSRAN_NOF_SF_X_FRAME)//每个子帧slot数*10
...
typedef enum SRSRAN_API {
  ...
  srsran_rnti_type_tc,     ///< @brief Contention Resolution (when no valid C-RNTI is available) (DL-SCH)竞争解决（当没有有效的C-RNTI可用时）（DL-SCH）
  srsran_rnti_type_cs,     ///< @brief Configured scheduled unicast transmission (DL-SCH, UL-SCH)配置的调度单播传输（DL-SCH、UL-SCH）
