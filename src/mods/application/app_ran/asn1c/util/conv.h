...
#define MCC_MNC_TO_TBCD(mCC, mNC, mNCdIGITlENGTH, tBCDsTRING)        \
		do {																 \
			char _buf[3];													 \
			 oset_assert((mNCdIGITlENGTH == 3) || (mNCdIGITlENGTH == 2));	   \
...
#define BIT_STRING_TO_TRANSPORT_LAYER_ADDRESS_IPv4(bITsTRING, mACRO)    \
		do {																	\
			oset_assert((bITsTRING)->size == 4);		  \
			oset_assert((bITsTRING)->bits_unused == 0); \
