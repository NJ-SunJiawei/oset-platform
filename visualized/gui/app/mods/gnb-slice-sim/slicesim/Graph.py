...
class Graph:
    ...
    def show_legend(self):
        ...
        #print(f'debug Client_{self.label}')
        for i in range(len(self.legend_indexed)):
    ...
    def draw_map(self):
        ...
        for c in self.clients:
            self.label = None
            if c.subscribed_slice_index not in self.legend_indexed and c.base_station is not None:
                self.label = c.get_slice().name
                self.legend_indexed.append(c.subscribed_slice_index)
            self.ax.scatter(c.x, c.y,
                            color=c.base_station.color if c.base_station is not None else '0.8',
                            label=self.label, s=15,
                            marker=markers[c.subscribed_slice_index % len(markers)])
            #print(f'debug Client_{c.pk} [{c.x:<5}, {c.y:>5}]')
            #print(f'debug legend_[{self.label}]')
