        ...
        self.text = {
            ...
            'NEW_FILE': 
                ("This is a simple text file editor intended to help with quick edits to assist users\n"
                 ...
                 "  '*.cmake'      - Define build oset target apps and files\n"
                 "  '*.c' - Define apps and libraries loaded during oset initialization\n\n"),
            ...
            'c':
                ("c/C++ defines which cFS libraries and apps are loaded during startup. The\n"
                "file's comment describe how to format/defince script entries. The following libraries\n"
                "and apps must be loaded\n"),       

            'JSON':
                ("JSON files are used by apps using the OpenSET app framework to define app initialization\n"
...
class TextEditor():
        ...
        if filename not in (None,''):
            if os.path.isfile(filename):
                ...
                with open(self.filename,'r') as f:
                    file_text = f.read()
                configurefile = filename.split('/')
                window['-FILE_TEXT-'].update(value=file_text)
                window['-FILE_TITLE-'].update(value=configurefile[-1])
        self.text_modified = False
    ...
    def create_window(self):
        ...
        menu_layout = [
                ...
                ['Edit',['!Select All','!Cut','&Copy','&Paste','---','!&Find...','!Replace...']],
                #['!Execute',['Build', 'Run Script']],
            ]

        self.file_text = sg.Multiline(default_text='', font=self.config.get('font'), enable_events=True, key='-FILE_TEXT-', size=(window_width,30), expand_x=True, expand_y=True,)
        
        window_layout = [
            [sg.Menu(menu_layout)],
            [[
              sg.Button('Help', enable_events=True, key='-FILE_HELP-', pad=((5,5),(12,12))),
              sg.Button('Save', enable_events=True, key='-FILE_SAVE-', pad=((5,5),(12,12))),
              sg.Text(self.NEW_FILE_STR, key='-FILE_TITLE-', font=self.config.get('font'), size=(window_width-10,1))
            ]],
            [sg.Column([[self.file_text]], expand_x=True, expand_y=True,)]]


        window = sg.Window('Configure File Editor', window_layout,
                                               resizable=True, 
    ...
    def gui(self):
        ...
        while True:
            ...
            elif encoded_event in ("b'Save'","b'Control_L:39'") or self.event == '-FILE_SAVE-': # s=39
                self.save_filename(window)
            ...
            ### Edit Menu ###
                ...
                window['-FILE_TEXT-'].Widget.selection_clear()
                window['-FILE_TEXT-'].Widget.tag_add("sel","1.0","end")
                ...
                try:
                    ...
                    window['-FILE_TEXT-'].Widget.insert(sg.tk.INSERT, clip)
