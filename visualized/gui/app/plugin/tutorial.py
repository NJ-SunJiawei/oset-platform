...
class Lesson():
    ...
    def gui(self):
        ...
        right_click_menu_def = [[],['&Prev', '&Next','---', '&Mark as Complete']]
        layout = [
                  [sg.Text('Slide'), sg.Text(str(self.cur_slide), pad=(1,1), key='-SLIDE-')],   
                  [sg.Image(filename=self.slide_file, key='-IMAGE-', expand_x=True, expand_y=True,)],
                  [sg.Button('Prev', pad=(2,1)), sg.Button('Next', pad=(2,1)), sg.Button('Mark as Complete', pad=(2,1))]
                 ]
 
        self.window = sg.Window(self.json.title(), layout, element_justification='c',
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        tutorial_rel_dir = '1-oset-overview'
