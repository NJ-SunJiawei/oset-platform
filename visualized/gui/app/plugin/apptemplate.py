...
class CreateApp():
    ...
    """
    def __init__(self, app_templates_path, app_relative_path):

        self.app_templates_path = app_templates_path
        self.app_relative_path = app_relative_path
                ...
                if self.selected_app is not None:
                    ...
                    if event == 'OK':
                        if values['-INPUT-'] is not None:
                            ...
                            if len(app_name) > 0:
                                app_created, new_app_dir = self.selected_app.create_app(app_name, os.path.join(os.getcwd(),self.app_relative_path))
                                if app_created:
